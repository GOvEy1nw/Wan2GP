...
class DownloadProgressTests(unittest.TestCase):
    def test_wgp_download_callbacks_are_optional_and_forwarded(self):
        tree = ast.parse(Path("wgp.py").read_text(encoding="utf-8"))
        functions = {node.name: node for node in tree.body if isinstance(node, ast.FunctionDef)}

        for name in ("process_files_def", "download_file", "download_models"):
            self.assertEqual(functions[name].args.args[-1].arg, "progress_callback")

        wrappers = {
            "process_files_def": "shared_process_files_def",
            "download_file": "shared_download_file",
        }
        for name, target in wrappers.items():
            call = next(node for node in ast.walk(functions[name]) if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id == target)
            self.assertTrue(any(keyword.arg == "progress_callback" and isinstance(keyword.value, ast.Name) and keyword.value.id == "progress_callback" for keyword in call.keywords))

        download_calls = [
            node for node in ast.walk(functions["download_models"])
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Name) and node.func.id in {"process_files_def", "download_file"}
        ]
        self.assertTrue(download_calls)
        self.assertTrue(all(any(keyword.arg == "progress_callback" and isinstance(keyword.value, ast.Name) and keyword.value.id == "progress_callback" for keyword in call.keywords) for call in download_calls))
